#!/usr/bin/env bash
#
# run_perf_profile.sh  --  microarchitectural profiling driver for CS683 PA-1 Task 1.
#
...
#
# src/perf_caliberation.cpp is profiled over exactly the same (H, W, K, seed)
# grid.  It performs the allocation / RNG fill / halo padding that every other
# binary performs and then stops, so its counters can be subtracted offline to
# isolate the kernel.  No subtraction is done here: this script only collects.
#
...
#     -o, --outdir DIR     output directory        (default: ./logs_perf_profile)
...
OUTDIR="$ROOT_DIR/logs_perf_profile"
...
#
# The variant strings are the function names inside each source file, exactly as
# in scripts/config.txt.  For the tile stage the variant is the tile size, which
# main() writes into the file-scope tile_size before calling conv_tile -- the
# same set of powers of two that run_tiling_speedup_sweep.sh used to sed in.
RUNS=(
    "calibration|default|calibration"

    "naive|default|naive"
    "reorder|default|reorder_base"

    "unroll|conv_unroll_v4|unroll_v4"
    "unroll|conv_unroll_v5|unroll_v5"

    "simd|conv_simd128_v2|simd128_v2"
    "simd|conv_simd128_v3|simd128_v3"
    "simd|conv_simd128_v4|simd128_v4"
    "simd|conv_simd256_v1|simd256_v1"
    "simd|conv_simd256_v2|simd256_v2"
    "simd|conv_simd256_v3|simd256_v3"
