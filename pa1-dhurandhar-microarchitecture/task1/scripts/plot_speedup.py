...
At each matrix size the plotted point is the MEAN speedup across the repeats
for that (stage, label); the error bars show +/- 1 sample standard deviation
across those repeats. If a run only has 1 repeat, its error bar is 0.

Usage:
    python plot_speedup.py --csv try2/summary.csv --outdir try2/plots
    python plot_speedup.py --csv try2/summary.csv --K 3
"""
...
def load_summary(csv_path: Path) -> pd.DataFrame:
    ...
    if n_bad:
        print(f"warning: {n_bad} row(s) in {csv_path} report INCORRECT output "
              f"and are still included in the mean/std")
...
def stats_for(df: pd.DataFrame, stage: str, label: str,
    ...
    """Mean & std of speedup per matrix size for one (stage, label[, K])."""
    ...
    g["std"] = g["std"].fillna(0.0)          # n == 1 -> NaN std -> 0
...
def _finish_axes(ax, xticklabels: List[str], title: str,
                  ylabel: str = "Speedup over naive (x)"):
