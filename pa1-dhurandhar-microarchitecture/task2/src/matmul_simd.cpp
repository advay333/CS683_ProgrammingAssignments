...
void matmul_simd256_v2(const float* A, const float* B, float* C,
                 int M, int N, int K, int lda, int ldb, int ldc) {
    // TODO(student): replace this placeholder with your register-tiled AVX2 implementation.
    const int simd_width = 8;
    const int unroll = 2;
    const int tail_k = K % simd_width;
    const int tail_n = N % unroll;
    for (int i = 0; i < M; ++i) {
        for (int j = 0; j < N - tail_n; j+= unroll) {
            ...
            for (int p = 0; p < K - tail_k; p+= simd_width) {
                __m256 va = _mm256_loadu_ps(&a[p]);
            ...
            __m128 msb0 = _mm256_extractf128_ps(acc0, 1);
            __m128 lsb0 = _mm256_castps256_ps128(acc0);
            __m128 sum0128 = _mm_add_ps(lsb0, msb0);
            sum0128 = _mm_hadd_ps(sum0128, sum0128);
            sum0128 = _mm_hadd_ps(sum0128, sum0128);


            __m128 msb1 = _mm256_extractf128_ps(acc1, 1);
            __m128 lsb1 = _mm256_castps256_ps128(acc1);
            __m128 sum1128 = _mm_add_ps(lsb1, msb1);
            sum1128 = _mm_hadd_ps(sum1128, sum1128);
            sum1128 = _mm_hadd_ps(sum1128, sum1128);

            float sum0 = _mm_cvtss_f32(sum0128);
            float sum1 = _mm_cvtss_f32(sum1128);


            for (int p = K - tail_k; p < K; ++p) {
                sum0 += a[p] * b0[p];
                sum1 += a[p] * b1[p];
            }
